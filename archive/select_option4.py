...
class polygon_drawer(object):
    def __init__(self, from_window_name, to_window_name):
        self.from_window_name = from_window_name # Name for our window
        self.to_window_name = to_window_name
    ...
    def run(self, from_image, to_image):
        # Let's create our working window and set a mouse callback to handle events
        # cv2.namedWindow(self.window_name, flags=cv2.WINDOW_AUTOSIZE)
        cv2.namedWindow(self.to_window_name)
        cv2.imshow(self.to_window_name, to_image)
        cv2.namedWindow(self.from_window_name)
        cv2.imshow(self.from_window_name, from_image)
        cv2.waitKey(1)
        cv2.setMouseCallback(self.from_window_name, self.on_mouse)

        temp_canvas = copy.deepcopy(from_image)
        ...
        while(not self.done):
            ...
            # Update the window
            cv2.imshow(self.from_window_name, temp_canvas)
            temp_canvas = copy.deepcopy(from_image)
            # And wait 50ms before next iteration (this will pump window messages meanwhile)
            if cv2.waitKey(50) == 27: # ESC hit
                self.done = True
        
        # overlay a filled polygon on the image
        overlay = copy.deepcopy(from_image)
        output = copy.deepcopy(from_image)
        cv2.fillPoly(overlay, np.array([self.points]), self.FILL_COLOR)
        alpha = 0.1
        cv2.addWeighted(overlay, alpha, from_image, (1-alpha), 0, output)
        cv2.imshow(self.from_window_name, output)
        ...
        cv2.waitKey()
        cv2.destroyWindow(self.from_window_name)
        cv2.destroyWindow(self.to_window_name)
