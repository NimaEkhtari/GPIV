...
class Piv:
    ...
    def compute(self, templateSize, stepSize, propFlag):
        ...
        for i in range(vCount):
            for j in range(uCount):
                ...
                if propFlag:
                    ...
                    peakCov[0][0] *= self.transform[0]*self.transform[0]
                    peakCov[0][1] *= self.transform[0]*self.transform[0]
                    peakCov[1][0] *= self.transform[0]*self.transform[0]
                    peakCov[1][1] *= self.transform[0]*self.transform[0]
        ...
        print(originUV.shape)
        print(self.transform[0])
        originUV *= self.transform[0] # scale by pixel ground size
        originUV[:,0] += self.transform[2] # offset U by leftmost pixel ground coordinate
        originUV[:,1] = self.transform[5] - originUV[:,1] # offset V by uppermost pixel ground coordinate
        offsetUV = np.squeeze(offsetUV)
        offsetUV *= self.transform[0] # scale by pixel ground size

        # export vector origins and offsets to 
    ...
    def _get_image_arrays(self):    
        ...
        # store the raster geometry
        self.transform = fromRaster.transform
